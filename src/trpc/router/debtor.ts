      ...
      try {
        ...
        if (responseCode >= 400) {
          if (responseCode === 404) {
            throw new TRPCError({
              code: 'NOT_FOUND',
              message: scfResponse?.data.msg || 'Veri bulunamadı.',
            });
          } else if (responseCode >= 400 && responseCode < 500) {
            throw new TRPCError({
              code: 'BAD_REQUEST',
              message: scfResponse?.data.msg || 'Geçersiz istek.',
            });
          } else {
            throw new TRPCError({
              code: 'INTERNAL_SERVER_ERROR',
              message: scfResponse?.data.msg || 'Sunucu hatası.',
            });
          }
        }
